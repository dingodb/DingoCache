...
| `DFKV_RDMA_RECV_SEGMENT_SIZE` | `128 GiB` hard budget | Maximum process receive-pool commitment; it is no longer allocated eagerly. Size for worst-case live/churn connections, but startup pins only one chunk. |
...
| `DFKV_RDMA_POOL_MAX` | `8` (default) | Maximum idle QPs retained per server and pool, across all rails. Scalar and SG operations share one exclusive-ownership data pool; control remains separate. Seven read workers plus one warm spare avoid the former 2× retention. |
