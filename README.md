...
| `DFKV_RDMA_ENDPOINT_CACHE_MAX` | `256` (default, process-wide) | Hard cap on live client RDMA endpoints across all `RdmaTransport` instances. New connections wait for a real destroy to return budget; idle-pool eviction alone does not release it. |
